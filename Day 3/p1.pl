...
my $point = 0;
foreach my $dirLines ( @ar ) {
    $point++;
    my $x = $dX;
    my $y = $dY;
    my @dirs = split(/\,/,$dirLines);
    foreach my $dir ( @dirs ) {
        my $compass = substr $dir, 0, 1;
        my $count = substr $dir, 1;
        for( my $i = 0; $i < $count; $i++ )  {
            ...
            $points{$point}->{"$modX,$modY"} = 1;
            if( $point == 2 && $points{1}->{"$modX,$modY"} == 1 ) {
                $intersecting{"$modX,$modY"} = 1;
            }
            $x = $modX;
            $y = $modY;
        }
...
my $minDist = undef;
foreach my $cond ( keys %intersecting ){
    my($x,$y) = split(/,/,$cond);
    #say "my $dist = calcDist($dX,$dY,$x,$y)";
    my $dist = calcDist($dX,$dY,$x,$y);
